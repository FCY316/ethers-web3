...
// 引入链参数配置
import { chainParams, evmChain } from '@/collocate';
 ...
 * 主要用于前端只读访问链上数据。
 */
const useChainRpc = create<AppState>((set) => {
  ...
  try {
    ...
    providerEvmReadOnly = new JsonRpcProvider(chainParams[evmChain[0]].rpcUrls[0]);

    // 创建 Router 合约只读实例
    routerAbiReadOnly = new Contract(
      evmCAddress[evmChain[0]].router.address,
      evmCAddress[evmChain[0]].router.abi,
      providerEvmReadOnly
    );
    // 创建 Factory 合约只读实例
    factoryAbiReadOnly = new Contract(
      evmCAddress[evmChain[0]].factory.address,
      evmCAddress[evmChain[0]].factory.abi,
      providerEvmReadOnly
    );
  } catch (err) {
    // 捕获初始化异常并记录错误信息
